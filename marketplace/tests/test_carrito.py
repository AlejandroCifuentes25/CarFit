...
class CarritoFrontendTest(TestCase):
    ...
    def test_comprar_el_carrito_procesa_todos_los_articulos(self):
        ...
        self.assertRedirects(respuesta, reverse("marketplace:historial_pagos"))
    ...
    def test_el_total_del_carrito_se_puede_consultar_como_resumen(self):
        ...
        respuesta = self.client.get(reverse("marketplace:calcular_total_carrito"))

        self.assertEqual(respuesta.status_code, 200)
        self.assertEqual(respuesta.json()["carrito"]["precio_total"], 85_000_000)
        self.assertEqual(respuesta.json()["carrito"]["cantidad_producto"], 1)